...
def encounters():
    ...
    """Return a list of dates for each prcp value"""
    # Query all dates and tobs
    results = session.query(Encounters.oshpd_id, Encounters.facility_name, Encounters.licensed_bed_size, Encounters.control_type_desc,  Encounters.Target, Encounters.LATITUDE, Encounters.LONGITUDE, Encounters.ED_Visit, Encounters.Medi_Cal, Encounters.Medicare, Encounters.Other_Payer, Encounters.SelfPay, Encounters.DX_Symptoms, Encounters.HispanicorLatino, Encounters.NonHis).\
    ...
    # Create a dictionary from the row data and append to a list of all_facilities
    all_encounters = []
    for id, name, bed, type, target, lat, lon,visits, medical, medicare, other, self, dx, his, non in results:
        encounters_dict = {}
